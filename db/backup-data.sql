...
-- Añadir nuevas columnas a la tabla `user` después de la columna `password`
ALTER TABLE `user`
ADD `fecha_registro` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER `password`,
ADD `ultima_conexion` TIMESTAMP NULL DEFAULT NULL AFTER `fecha_registro`;
